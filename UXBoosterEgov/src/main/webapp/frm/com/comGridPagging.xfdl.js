    ...
    return function()
    {
        ...
        */
        ...
        this.fnSearchCall = function(nPage)
        {
            ...
            //데이터셋 curpage 속성 값에 현재 페이지 세팅
            objBindDs["curpage"] = nPage;
        ...
        this.fnSetting = function()
        {
            ...
            //전체 row수
            sTotalRow = take.nvl(objDs["totalcnt"], take.nvl(objDs.getConstColumn("TOTALCNT"), ""));

            //전체 rows가 없으면 리턴
            if (sTotalRow=="")
            {
                take.debug("comGridPagging.fnSetting", "바인드된 데이터셋("+objDs.name+")에 totalcnt 또는 TOTALCNT(Constcolumn)값이 없습니다.");
                this.fv_nTotalRow = -1;
                return false;
            ...
            //한페이지에 표기할 row수
            sRowCnt = take.nvl(objDs["rowcnt"], take.nvl(objDs.getConstColumn("ROWCNT"), ""));

            //한페이지에 표기할 row수가 없을 경우 리턴
            if (sRowCnt=="")
            {
                take.debug("comGridPagging.fnSetting", "바인드된 데이터셋("+objDs.name+")에 rowcnt 또는 ROWCNT(Constcolumn)값이 없어 기본값(10)으로 세팅합니다.");
                this.fv_nRowCnt = 10;
